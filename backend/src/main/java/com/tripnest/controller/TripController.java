...
public class TripController {
    ...
    public ResponseEntity<com.tripnest.dto.MessageResponse> shareTrip(
            @AuthenticationPrincipal CustomUserPrincipal principal,
            @PathVariable Long tripId,
            @RequestBody com.tripnest.dto.ShareTripRequest request
    ) {
        return ResponseEntity.ok(tripService.shareTrip(principal.getId(), tripId, request.getEmail()));
    }

    // GET /api/trips/:tripId/timeline
    @GetMapping("/{tripId}/timeline")
    public ResponseEntity<com.tripnest.dto.TripTimelineResponse> getTripTimeline(
            @AuthenticationPrincipal CustomUserPrincipal principal,
            @PathVariable Long tripId
    ) {
        return ResponseEntity.ok(tripService.getTripTimeline(principal.getId(), tripId));
    }
}